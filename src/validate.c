...
void makedepthmapforbfs(const size_t nlocalverts,const int64_t root,int64_t * const pred,float* dist) {
	...
	while(newvisits!=0) {
		...
		for(i=0;i<nlocalverts;i++)
			if(dist[i]==prevlevel)
				for(int64_t j=vrowstarts[i];j<vrowstarts[i+1];j++)
...
vweights=weights;
#endif
#else
		vdegrees=xcalloc(nlocalverts,sizeof(*vdegrees));
		...
		vrowstarts = xmalloc((nlocalverts + 1) * sizeof(*vdegrees));
